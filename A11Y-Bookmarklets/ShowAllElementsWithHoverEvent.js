...
function createA11YResultTableRowHeader() {
	let details = ["ID", "Element Name", "Element Type", "SRC", "Action"];
...
function mainFunction() {
	...
	let elementsWithMouseOver = [];
	
	elements.forEach(element => {
		if (getEventListeners(element).mouseover) {
	...
	for (let i=0; i<elementsWithMouseOver.length; i++) {
		let details = [];
		
		details.push(elementsWithMouseOver[i].innerText);
		details.push(elementsWithMouseOver[i]);
		details.push(elementsWithMouseOver[i].parentNode.innerHTML);
		createA11YResultTableRowBody(details, elementsWithMouseOver[i]);
		count++;
	}
